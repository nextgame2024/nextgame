 ...
 */
class RatingsCentralRepository extends ServiceEntityRepository
{
    ...
    public function findGamesByLocationId(
        ...
        if (empty($status) || empty($searchBy) || empty($searchValue) || empty($searchDate)) {
            return [];
        }
        ...
        $sql = 'SELECT 
            ...
            WHERE g.location_id = :locationId AND g.status = :status';
